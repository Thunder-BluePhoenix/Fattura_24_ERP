...
# @frappe.whitelist()
# def send_invoice_to_fattura24_bulkly(bulk_doc_name, method=None):
#     bulk_invoice_doc = frappe.get_doc("Bulk Invoice Log", bulk_doc_name)
#     settings = frappe.get_doc("Fattura24 Settings")
#     if not settings.enabled:
#         return
    
#     number_of_invoices_tobe_send_bulkly = int(settings.number_of_invoices)
    
#     # Get pending invoices (not sent and with valid invoice_id)
#     pending_invoices = [inv for inv in bulk_invoice_doc.customer_details 
#                        if inv.sent_to_fattura24 != 1 and inv.invoice_id is not None]
    
#     # Determine how many invoices to process (minimum of limit or available invoices)
#     invoices_to_process = min(number_of_invoices_tobe_send_bulkly, len(pending_invoices))
    
#     # Process only the determined number of invoices
#     for i in range(invoices_to_process):
#         invoice = pending_invoices[i]
#         doc = frappe.get_doc("Sales Invoice", invoice.invoice_id)
#         send_invoice_to_fattura24(doc, method=None)
#         invoice.sent_to_fattura24 = 1

#     bulk_invoice_doc.save()
    
#     # Return information about the processing
#     return {
#         "invoices_processed": invoices_to_process,
#         "total_pending": len(pending_invoices),
#         "limit": number_of_invoices_tobe_send_bulkly
#     }


@frappe.whitelist()
def send_invoice_to_fattura24_funnel_triggered(docname, method=None):
    # if isinstance(doc, str):
    #     # If only the name is passed, fetch the full doc
    doc = frappe.get_doc("Bulk Invoice Log", docname)  # replace with your actual doctype

    doc.bulk_send_triggered = True
    doc.submit()
...
def send_invoice_to_fattura24_bulkly_funnel(variables=None):
   
    if not variables or not variables.get("doc"):
        frappe.throw("Document reference not provided in variables")
    bulk_doc = variables["doc"]
    # doc = "CRM-LEAD-2024-00066"
    bulk_invoice_doc = frappe.get_doc("Bulk Invoice Log", bulk_doc["name"])
    # bulk_invoice_doc = frappe.get_doc("Bulk Invoice Log", bulk_doc_name)
    settings = frappe.get_doc("Fattura24 Settings")
    if not settings.enabled:
        return
    ...
    count = 0
    for invoice in bulk_invoice_doc.customer_details :
        if invoice.sent_to_fattura24 != 1 and invoice.invoice_id is not None:
            count += 1

    bulk_invoice_doc.invoices_to_be_sent_to_fattura24 = count
